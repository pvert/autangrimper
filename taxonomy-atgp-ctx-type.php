...
$affichage_portfolio = get_field('atgp_affichage_en_portfolio', $term);
?>
...
<div id="content-area" class="wrapper archives">
	...
	if($affichage_portfolio != true) { // if portfolio then display it, if not just do the normal script
        ...
        //atgp_get_associate_content_arguments($title, $type_of_content, $taxonomy, $post_number, $post_offset, $className)
        $className="";
        $layout="";
        $show_more="";
        $show_more_text="";
        foreach ($whatToShow as $display) {
            atgp_get_associate_content_arguments($display['title'], $display['posttype'], $display['taxonomy'], $display['post_number'], $display['post_offset'], $layout, $className, $show_more, $show_more_text );
        }
    } else {
        ?>
        <script>
        ...
        <style>
            .atgp_port_action_btn li:hover {
                background-color: <?php echo $code_color; ?> !important;
            }

            .atgp_port_action_btn li.active {
                background-color: <?php echo $code_color; ?> !important;
            }
        </style>
        <?php 
